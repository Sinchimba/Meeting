...
const smsLogRepo = () => data_source_1.AppDataSource.getRepository(sms_log_entity_1.SmsLog);
class SmsService {
    /**
     ...
     */
    async sendSms(rawPhone, message) {
        let formattedPhone;
        try {
    ...
    sendViaTwilio(to, body) {
        ...
        return new Promise((resolve, reject) => {
            const postData = querystring_1.default.stringify({ To: to, From: from, Body: body });
            ...
            const req = https_1.default.request(options, (res) => {
                ...
                res.on('end', () => {
                    ...
                    if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
                        ...
                        const err = new Error(parsed?.message || `Twilio API returned HTTP ${res.statusCode}`);
    ...
    sendViaAfricasTalking(to, body) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const postData = querystring_1.default.stringify(payload);
            const options = {
                    ...
                    if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
                        ...
                        if (recipients && recipients.length > 0) {
                            ...
                            if (status === 'Success') {
                                ...
                                const err = new Error(`Africa's Talking gateway error status: ${status}`);
                                err.responseBody = responseBody;
                                reject(err);
                            }
                        }
                        ...
                        const err = new Error(`Africa's Talking API returned HTTP ${res.statusCode}`);
                        err.responseBody = responseBody;
                        reject(err);
                    }
                });
