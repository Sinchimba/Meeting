...
import { Video, Users, Mic, MicOff, VideoOff, Copy, PhoneOff, Send, MessageSquare, HandMetal, CheckCircle2, AlertCircle, Clock } from 'lucide-react'
...
export default function MeetingRoom() {
        ...
        const fetchMeeting = async () => {
          try {
            ...
            if (meeting) {
              ...
              setUserRole(roleParam || parsedUser.role || 'normal')
            }
          } catch (err) {
  ...
  }, [meetingId, searchParams, navigate])
  ...
  useEffect(() => {
    ...
    newSocket.on('current-participants', (data) => {
      console.log('Current participants list received:', data)
    ...
    const initWebRTC = async () => {
      try {
        ...
        if (webrtcService.localStream && localVideoRef.current) {
          ...
          if (userRole === 'standard' || userRole === 'normal') {
  ...
  useEffect(() => {
    ...
    participants.forEach(participant => {
      ...
      webrtc.getRemoteStream(participant.socketId, (userId, stream) => {
      ...
      <div className="meeting-layout">
        ...
        <div className="sidebar">
          ...
          <div className="panel ai-panel glass-panel">
            ...
            {userRole !== 'standard' && userRole !== 'normal' && (
          ...
          <div className="panel chat-panel glass-panel">
            <h3><MessageSquare size={18} style={{ marginRight: '8px', verticalAlign: 'middle' }} /> Chat</h3>
            <div className="messages">
              {messages.map((msg, idx) => {
                ...
                return (
                  <div key={idx} className={`message ${msg.userName === user?.name ? 'self' : 'other'}`}>
                    <strong className="message-author">{msg.userName}</strong>
                    <p className="message-text">{msg.message}</p>
